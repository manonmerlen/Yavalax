...
class Gui extends OpenXum.Gui {
    ...
    _on_click(event) {
        ...
        if (this.engine()._coup_valide(Math.trunc(c[0]), Math.trunc(c[1]), this.engine().current_color())){
            ...
            for (let i = 0; i < 13; i++) {
                for (let j = 0; j < 13; j++) {
                    if (!this.engine()._coup_valide(i, j, this.engine().current_color()) && this.engine()._case_libre(i,j)) {
                        this._context.strokeStyle = "#778899";
                        this._draw_croix_erreur(i, j);
                    }

                    if(this.engine()._coup_valide(i, j, this.engine().current_color())  && this.engine()._case_libre(i,j))
                    {
                        //this._context.clearRect(i, j,  , );
                        //croix rose
    ...
    _draw_carre_1(couleur) {
        ...
        this._context.moveTo(this._offsetX + (pt[0] + 0.1) * this._deltaX,
            this._offsetY + (pt[1] + 0.1) * this._deltaY);
        this._context.lineTo(this._offsetX + (pt[0] + 0.5) * this._deltaX - 2,
            this._offsetY + (pt[1] + 0.1) * this._deltaY);
        this._context.lineTo(this._offsetX + (pt[0] + 0.5) * this._deltaX - 2,
            this._offsetY + (pt[1] + 0.5) * this._deltaY - 2);
        this._context.lineTo(this._offsetX + (pt[0] + 0.1) * this._deltaX,
            this._offsetY + (pt[1] + 0.5) * this._deltaY - 2);
        this._context.moveTo(this._offsetX + (pt[0] + 0.1) * this._deltaX,
            this._offsetY + (pt[1] + 0.1) * this._deltaY);
        this._context.closePath();
    ...
    _draw_triangle() {
        ...
        this._context.beginPath();
        this._context.moveTo(this._offsetX + this._deltaX * (pt[0] + 0.1),
            this._offsetY + this._deltaY * (pt[1] - 0.25));
        this._context.lineTo(this._offsetX + this._deltaX * (pt[0] + 0.3),
            this._offsetY + this._deltaY * (pt[1]));
        this._context.lineTo(this._offsetX + this._deltaX * (pt[0] + 0.5),
            this._offsetY + this._deltaY * (pt[1] - 0.25));
        this._context.moveTo(this._offsetX + this._deltaX * (pt[0] + 0.1),
            this._offsetY + this._deltaY * (pt[1] - 0.25));
        this._context.closePath();
