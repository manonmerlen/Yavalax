...
class Player {
  ...
  move() {
    ...
    for (let coup = 0; coup < possibleMoves.length; coup++) {
      ...
      e.move(move);
      let tmpMax = this._profundity(e, this._depth - 1,50,-50, this._color, move);
      e.undo_move(move);
  ...
  _evaluate(e, c, column, line) {
    ...
    this._actualise_case_interest(this._engine, c);
    this._actualise_case_disinterest(this._engine, c);
    this._actualise_case_interest(this._engine, c);

    let center = Player._center_points(column, line);
    let follow = Player._follow(e, c, column, line);
    let interest = this._near_interest(e, column, line);//a côté d'une case à vic potentiel
...
//l'algo n'arrive pas ici, on a pas réussi à bien faire fonctionner l'IA en profondeur
      let b2 = e.clone();
      let possibleMoves = b2.get_possible_move_list();
      for (let i = 0; i < possibleMoves.length; i++) {
        let move = possibleMoves[i];
        b2.move(move);
        let tmp = -this._profundity(b2, this._depth - 1, a, b, this._color, move);
        b2.undo_move(move);
